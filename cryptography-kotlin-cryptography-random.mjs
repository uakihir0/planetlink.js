...
class Default extends CryptographyRandom {
  constructor() {
    return new.target.fnb();
  }
  static fnb() {
    Default_instance = null;
    var $this = this.cnb();
    Default_instance = $this;
    $this.enb_1 = defaultCryptographyRandom();
    return $this;
  }
  ir(bitCount) {
    return this.enb_1.ir(bitCount);
  }
  zm() {
    return this.enb_1.zm();
  }
  v2(until) {
    return this.enb_1.v2(until);
  }
  jr(from, until) {
    return this.enb_1.jr(from, until);
  }
  kr() {
    return this.enb_1.kr();
  }
  lr(until) {
    return this.enb_1.lr(until);
  }
  mr(from, until) {
    return this.enb_1.mr(from, until);
  }
  nr() {
    return this.enb_1.nr();
  }
  or(array) {
    return this.enb_1.or(array);
  }
  pr(size) {
    return this.enb_1.pr(size);
  }
  qr(array, fromIndex, toIndex) {
    return this.enb_1.qr(array, fromIndex, toIndex);
  }
}
class WebCryptoCryptographyRandom extends AbstractRandom {
  constructor() {
    return new.target.inb();
  }
  static inb() {
...
function fillBytes($this, jsArray) {
  ...
  if (size <= 65536) {
    $this.hnb_1.getRandomValues(jsArray);
  } else {
    var filled = 0;
    do {
      ...
      $this.hnb_1.getRandomValues(jsArray.subarray(filled, filled + chunkSize | 0));
...
function WebCryptoCryptographyRandom_getInstance() {
  if (WebCryptoCryptographyRandom_instance === VOID)
    WebCryptoCryptographyRandom.inb();

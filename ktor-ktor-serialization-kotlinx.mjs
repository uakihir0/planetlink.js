...
class sam$kotlinx_coroutines_flow_FlowCollector$0 {
  constructor(function_0) {
    this.tks_1 = function_0;
  }
  *n2n(value, $completion) {
    return yield* this.tks_1(value, $completion);
  }
  c5() {
    return this.tks_1;
  }
...
class KotlinxSerializationConverter$serialize$$inlined$map$1 {
  constructor($this, $contentType, $charset, $typeInfo, $value) {
    this.uks_1 = $this;
    this.vks_1 = $contentType;
    this.wks_1 = $charset;
    this.xks_1 = $typeInfo;
    this.yks_1 = $value;
  }
  *m2n(collector, $completion) {
    var tmp = KotlinxSerializationConverter$serialize$o$collect$slambda(collector, this.vks_1, this.wks_1, this.xks_1, this.yks_1);
    yield* this.uks_1.k2n(new sam$kotlinx_coroutines_flow_FlowCollector$0(tmp), $completion);
...
class KotlinxSerializationConverter {
  constructor(format) {
    this.zks_1 = format;
    this.akt_1 = extensions(this.zks_1);
    var tmp;
    var tmp_0 = this.zks_1;
    if (isInterface(tmp_0, BinaryFormat)) {
      tmp = true;
    } else {
      var tmp_1 = this.zks_1;
      tmp = isInterface(tmp_1, StringFormat);
    }
    // Inline function 'kotlin.require' call
    if (!tmp) {
      var message = 'Only binary and string formats are supported, ' + toString(this.zks_1) + ' is not supported.';
      throw IllegalArgumentException.a2(toString(message));
    }
  }
  *bkt(contentType, charset, typeInfo, value, $completion) {
    // Inline function 'kotlinx.coroutines.flow.map' call
    // Inline function 'kotlinx.coroutines.flow.unsafeTransform' call
    var this_0 = asFlow(this.akt_1);
    ...
    var tmp_0;
    try {
      tmp_0 = serializerForTypeInfo(this.zks_1.rz(), typeInfo);
    } catch ($p) {
      var tmp_1;
      if ($p instanceof SerializationException) {
        var cause = $p;
        tmp_1 = guessSerializer(value, this.zks_1.rz());
      } else {
        throw $p;
      }
      tmp_0 = tmp_1;
    ...
    return serializeContent(this, serializer, this.zks_1, value, contentType, charset);
  }
  rks(contentType, charset, typeInfo, value, $completion) {
    return this.bkt(contentType, charset, typeInfo, value, $completion);
  }
  *m4k(charset, typeInfo, content, $completion) {
    var contentPacket = yield* readRemaining(content, $completion);
    var _iterator__ex2g4s = this.akt_1.l1();
    $l$loop: while (_iterator__ex2g4s.m1()) {
      var ext = _iterator__ex2g4s.n1();
      var tmp0_elvis_lhs = yield* ext.m4k(charset, typeInfo, ByteReadChannel(contentPacket), $completion);
      var tmp;
    ...
    var serializer = serializerForTypeInfo(this.zks_1.rz(), typeInfo);
    try {
      var tmp1_subject = this.zks_1;
      var tmp_0;
      if (isInterface(tmp1_subject, StringFormat)) {
        tmp_0 = this.zks_1.s10(serializer, readText(contentPacket, charset));
      } else {
        if (isInterface(tmp1_subject, BinaryFormat)) {
          tmp_0 = this.zks_1.u10(serializer, readByteArray(contentPacket));
        } else {
          discard(contentPacket);
          // Inline function 'kotlin.error' call
          var message = 'Unsupported format ' + toString(this.zks_1);
      ...
      if ($p instanceof Error) {
        var cause = $p;
        throw JsonConvertException.f4k('Illegal input: ' + cause.message, cause);
      } else {
        throw $p;
      }
    }
...
function extensions(format) {
  ...
  while (_iterator__ex2g4s.m1()) {
    ...
    var tmp0_safe_receiver = element.sks(format);
...
function serialization(_this__u8e3s4, contentType, format) {
  _this__u8e3s4.k4k(contentType, new KotlinxSerializationConverter(format));
}
function KotlinxSerializationConverter$serialize$o$collect$slambda($$this$unsafeFlow, $contentType, $charset, $typeInfo, $value) {
  return constructCallableReference(function *(value, $completion) {
    ...
    var tmp$ret$1 = yield* value.rks($contentType, $charset, $typeInfo, $value, $completion);
...
function serializeContent($this, serializer, format, value, contentType, charset) {
  ...
  if (isInterface(format, StringFormat)) {
    var content = format.r10(isInterface(serializer, KSerializer) ? serializer : THROW_CCE(), value);
    tmp = new TextContent(content, withCharsetIfNeeded(contentType, charset));
  } else {
    if (isInterface(format, BinaryFormat)) {
      var content_0 = format.t10(isInterface(serializer, KSerializer) ? serializer : THROW_CCE(), value);
...
function serializerForTypeInfo(_this__u8e3s4, typeInfo) {
  ...
  if (tmp1_elvis_lhs == null) {
    var tmp2_safe_receiver = module_0.x11(typeInfo.h3w_1);
    tmp_1 = tmp2_safe_receiver == null ? null : maybeNullable(tmp2_safe_receiver, typeInfo);
  } else {
    tmp_1 = tmp1_elvis_lhs;
  }
  var tmp3_elvis_lhs = tmp_1;
  return tmp3_elvis_lhs == null ? maybeNullable(serializer(typeInfo.h3w_1), typeInfo) : tmp3_elvis_lhs;
}
      ...
      if (isArray(value)) {
        ...
        if (isInterface(value, KtSet)) {
          ...
          if (isInterface(value, KtMap)) {
            ...
            var tmp3_elvis_lhs = module_0.x11(getKClassFromExpression(value));
...
function checkTypeParameters(type, typeInfo, module_0) {
  ...
  while (_iterator__ex2g4s.m1()) {
    ...
    var tmp;
    try {
      var tmp0_safe_receiver = element.us_1;
      var tmp_0;
      if (tmp0_safe_receiver == null) {
        tmp_0 = null;
      } else {
  ...
  var s = _destruct__k2r9zo.pm();
  var be = _destruct__k2r9zo.qm();
  throw SerializationException.h11('Serializer for type argument' + s + ' ' + argNames + ' ' + be + " not found for '" + typeInfo.h3w_1.f1() + "'. " + ('Ensure that the listed type' + s + ' ' + be + " marked as '@Serializable'."));
}
